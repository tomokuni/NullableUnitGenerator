...
/// <summary>
/// 3状態を表現するジェネリック版インターフェイス ITernaryState&lt;T&gt; (UNDEF, NULL, Value)

/// </summary>
public interface ITernaryState<T> : IEquatable<ITernaryState<T>>, ITernaryState
{
    //
    // Constructor
    //
    ...
    public static ITernaryState<T> UndefValue { get; }
    ...
    /// </returns>
    public static byte CheckState(in ITernaryState<T> x, in ITernaryState<T> y)
        => (x, y) switch
        {
    ...
    /// </returns>
    public static byte CheckState(in ITernaryState<T> x)
        => x switch
        {

...
/// </summary>
public readonly struct TernaryStruct<T> : IEquatable<TernaryStruct<T>>, IEqualityComparer<TernaryStruct<T>>, ITernaryType<T> where T : struct, IEquatable<T>
{
    //
    // Constructor
    //
    ...
    /// <summary>Complete Constructor</summary>
    public TernaryStruct(TernaryState state, [AllowNull] T value = default)
    ...
    /// <summary>Complete Constructor</summary>
    public TernaryStruct([AllowNull] T? value)
    ...
    /// <returns><b>value</b> : if assigned and not null<br/><b>defaultValue</b> : otherwise</returns>
    [return: MaybeNull]
    public T? GetOr([AllowNull] T? defaultValue, bool treatUndefAsNull = false)
        => HasValue ? m_value : ((IsNull || treatUndefAsNull) ? defaultValue : throw new InvalidOperationException(DISPLAY_STRING_AS_UNDEF));

    public T GetOr([DisallowNull] T defaultValue, bool treatUndefAsNull = false)
        => HasValue ? m_value : ((IsNull || treatUndefAsNull) ? defaultValue : throw new InvalidOperationException(DISPLAY_STRING_AS_UNDEF));
    ...
    public bool Equals(ITernaryType<T>? other)
        => other is TernaryStruct<T> ts ? Equals(ts) : base.Equals(other);

    public bool Equals(ITernaryType<T>? x, ITernaryType<T>? y)
        => x.Equals(y.State) && x.Equals(y.GetRawValue());
